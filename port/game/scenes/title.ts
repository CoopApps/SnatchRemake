// Scene: SNATCHER title screen — full-plane graphic (896 cells / 359 unique
// tiles / 6 blocks). TRUE PORT — bundled assets only.

import { Screen, type RgbFrame, type Palette } from '../render/screen.ts';
import { loadScene, wordToRgb } from '../assets/loader.ts';
import { TitleSequence } from '../engine/title_seq.ts';
...
let cached: { upTo: number; seq: TitleSequence } | null = null;
function seqAt(f: number, pw: number[][]): TitleSequence {
  if (!cached || cached.upTo > f || cached.seq.done) cached = { upTo: 0, seq: new TitleSequence(pw) };
...
export const title = {
  ...
  render(frame: number): RgbFrame {
    ...
    for (const cell of layout.cells) {
      const tile = assets.tileBlocks[cell.block]?.[cell.tile];
      if (!tile) continue;
