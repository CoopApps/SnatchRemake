// Iterative plane-B hunt: all tile blocks + nametable + palette for plane B.
// ORACLE use only — capture guides the disc lookup; the port ships disc bytes.
//   npx tsx port/build/hunt_pb.ts <frame>
...
// Plane B: scan full 64x32 (plane size handles wrap; we only need cols 0..31 rows 0..27 for display but grab all for tiles).
const ntB = (reg[4] & 0x07) << 13;
const hCells = [32, 64, 32, 128][reg[16] & 3];
const used = new Set<number>();
// Grab plane-B nametable cells for the visible 32x28 window (matches renderer).
const cells: { c: number; r: number; tile: number; pal: number; hf: number; vf: number; pri: number }[] = [];
for (let r = 0; r < 28; r++) for (let c = 0; c < 32; c++) {
  const w = vram[ntB + (r * hCells + c) * 2] | (vram[ntB + (r * hCells + c) * 2 + 1] << 8);
  const tile = w & 0x7ff;
  if (tile === 0) continue;
  used.add(tile);
  cells.push({ c, r, tile, pal: (w >> 13) & 3, hf: (w >> 11) & 1, vf: (w >> 12) & 1, pri: (w >> 15) & 1 });
}
console.log(`f${frame} plane B: ${used.size} tiles, ${cells.length} cells (hCells=${hCells})`);
if (!used.size) { console.log('  no plane B'); process.exit(0); }
...
for (const e of idxE) {
  try { const dc = Buffer.from(decompressLzss(dfc.get(e.file)!, e.off + 2, 0x20000)); if (dc.length < 64) continue; const sw = Buffer.alloc(dc.length); for (let i = 0; i + 1 < dc.length; i += 2) { sw[i] = dc[i + 1]; sw[i + 1] = dc[i]; } cache.push({ file: e.file, off: e.off, size: e.size, sw }); } catch { }
}

let missing = [...used];
const found: { file: string; off: number; base: number }[] = [];
for (let round = 0; round < 8 && missing.length; round++) {
  const probes = missing.map(t => { let nz = 0; for (let i = 0; i < 32; i++) if (vram[t * 32 + i] !== 0) nz++; return { t, nz }; }).sort((a, b) => b.nz - a.nz).slice(0, 4);
  ...
  if (!best || !best.covers.size) { console.log(`  ${missing.length} unpinned`); break; }
...
for (let r = 0; r < 28; r++) { const be = Buffer.alloc(64); const t = new Set<number>(); for (let c = 0; c < 32; c++) { const lo = vram[ntB + (r * hCells + c) * 2], hi = vram[ntB + (r * hCells + c) * 2 + 1]; be[c * 2] = hi; be[c * 2 + 1] = lo; t.add(((hi << 8) | lo) & 0x7ff); } nrows.push({ r, distinct: t.size, be }); }
nrows.sort((a, b) => b.distinct - a.distinct);
let ntHit = '';
for (const b of cache) { const orig = Buffer.alloc(b.sw.length); for (let i = 0; i + 1 < b.sw.length; i += 2) { orig[i] = b.sw[i + 1]; orig[i + 1] = b.sw[i]; } let n = 0; for (const p of nrows.slice(0, 3)) if (orig.indexOf(p.be) >= 0) n++; if (n >= 2) { ntHit = `${b.file} @$${b.off.toString(16)}`; break; } }
console.log(`  tiles ${found.length} blocks, ${missing.length} unpinned; NT ${ntHit || 'NOT FOUND'}`);
if (!missing.length && ntHit) {
  console.log('  READY planeB:');
  console.log('    tiles: [');
  found.forEach((b, i) => console.log(`      { name: '${i === 0 ? 'B' : 'B' + (i + 1)}', file: '${b.file}', offset: 0x${b.off.toString(16)}, compression: 'lzss', sizePrefix: true, loadTileBase: 0x${b.base.toString(16)} },`));
  console.log(`    ],`);
  console.log(`    nametable: { file: '${ntHit.split(' @$')[0]}', offset: 0x${ntHit.split('@$')[1]}, compression: 'lzss', sizePrefix: true, loadTileBase: 0x${found[0].base.toString(16)} },`);
}
